...
/**
 * This class defines the structure of the 'maps' table.
 *
 *
 *
 ...
 */
class MapsTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = '.Map.MapsTableMap';
     ...
     */
    const COL_ID = 'maps.id';

    /**
     * the column name for the file field
     */
    const COL_FILE = 'maps.file';

    /**
     * the column name for the name field
     */
    const COL_NAME = 'maps.name';

    /**
     * the column name for the imgurl field
     */
    const COL_IMGURL = 'maps.imgurl';
    ...
    public function initialize()
    {
        // attributes
        $this->setName('maps');
        $this->setPhpName('Maps');
        $this->setIdentifierQuoting(false);
        $this->setClassName('\\Maps');
        $this->setPackage('');
    ...
    {
        return $withPrefix ? MapsTableMap::CLASS_DEFAULT : MapsTableMap::OM_CLASS;
    }
     ...
     * @return array           (Maps object, last column rank)
     */
    public static function populateObject($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        $key = MapsTableMap::getPrimaryKeyHashFromRow($row, $offset, $indexType);
        if (null !== ($obj = MapsTableMap::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $offset, true); // rehydrate
            $col = $offset + MapsTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = MapsTableMap::OM_CLASS;
            /** @var Maps $obj */
            $obj = new $cls();
            $col = $obj->hydrate($row, $offset, false, $indexType);
            MapsTableMap::addInstanceToPool($obj, $key);
        }
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = MapsTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = MapsTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    {
        if (null === $alias) {
            $criteria->addSelectColumn(MapsTableMap::COL_ID);
            $criteria->addSelectColumn(MapsTableMap::COL_FILE);
    ...
    /**
     * Performs a DELETE on the database, given a Maps or Criteria object OR a primary key value.
     *
     * @param mixed               $values Criteria or Maps object or primary key or array of primary keys
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \Maps) { // it's a model object
            // create criteria based on pk values
            $criteria = $values->buildPkeyCriteria();
        } else { // it's a primary key, or an array of pks
            $criteria = new Criteria(MapsTableMap::DATABASE_NAME);
            $criteria->add(MapsTableMap::COL_ID, (array) $values, Criteria::IN);
        }

        $query = MapsQuery::create()->mergeWith($criteria);

        if ($values instanceof Criteria) {
            MapsTableMap::clearInstancePool();
        } elseif (!is_object($values)) { // it's a primary key, or an array of pks
            foreach ((array) $values as $singleval) {
                MapsTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    {
        return MapsQuery::create()->doDeleteAll($con);
    }

    /**
     * Performs an INSERT on the database, given a Maps or Criteria object.
     *
     * @param mixed               $criteria Criteria or Maps object containing data that is used to create the INSERT statement.
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria instanceof Criteria) {
            ...
            $criteria = $criteria->buildCriteria(); // build Criteria from Maps object
        }

        if ($criteria->containsKey(MapsTableMap::COL_ID) && $criteria->keyContainsValue(MapsTableMap::COL_ID) ) {
